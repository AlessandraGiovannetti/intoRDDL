...
# --------------------------------------------------------------------------
# Selezione interattiva dello stato iniziale
# --------------------------------------------------------------------------

def coerce_bool_series(series):
    """Converte una colonna in booleani Python robusti, indipendentemente da
    come e' stata letta da pandas (bool nativo, stringhe 'True'/'False',
    0/1, NaN -> False)."""
    def _to_bool(v):
        ...
        if isinstance(v, (int, np.integer, float, np.floating)):
            return bool(v)
        s = str(v).strip().lower()
        return s in ('true', '1', 'yes', 'si', 'y')
    return series.map(_to_bool)


def choose_init_state(states_df, id_col, initial_col, explicit_init_state):
    """Determina lo stato iniziale da usare in instance.rddl.

    Priorita':
      1. --init-state esplicito (nessuna interazione).
      2. Colonna `initial_col` presente nel CSV degli stati: gli stati con
         flag True sono candidati. Se ce n'e' uno solo, viene scelto
         automaticamente; se ce ne sono piu' d'uno, vengono stampati a
         console e l'utente sceglie interattivamente (input()); se non ce
         n'e' nessuno, si passa al fallback 3.
      3. Fallback storico: colonna 'start' su last_action_col, altrimenti
         stato 0.
    """
    if explicit_init_state is not None:
        return explicit_init_state

    if initial_col in states_df.columns:
        flags = coerce_bool_series(states_df[initial_col])
        candidates = states_df.loc[flags, id_col].astype(int).tolist()

        if len(candidates) == 1:
            chosen = candidates[0]
            print(f"Stato iniziale (unico candidato flaggato in '{initial_col}'): s{chosen}")
            return chosen

        if len(candidates) > 1:
            print(f"\nStati iniziali disponibili (colonna '{initial_col}'):")
            for c in candidates:
                print(f"  [{c}] s{c}")
            while True:
                raw = input(f"Scegli lo stato iniziale da cui partire {candidates}: ").strip()
                try:
                    chosen = int(raw)
                except ValueError:
                    print("Input non valido, inserisci uno degli id numerici mostrati sopra.")
                    continue
                if chosen not in candidates:
                    print(f"'{chosen}' non e' tra gli stati candidati {candidates}, riprova.")
                    continue
                return chosen

    return None  # nessun candidato trovato: si passa al fallback nel chiamante
...
def main():
    ...
    ap.add_argument('--states', required=True,
                     help="nome (o percorso) del CSV degli stati; se relativo, viene risolto "
                          "rispetto a --indir.")
    ap.add_argument('--transitions', required=True,
                     help="nome (o percorso) del CSV delle transizioni; se relativo, viene "
                          "risolto rispetto a --indir.")
    ap.add_argument('--indir', default='./src/output',
                     help="cartella in cui cercare i CSV di input (--states/--transitions), se "
                          "questi sono passati come nomi relativi. Default: ./src/output")
    ap.add_argument('--outdir', default='./src/output',
                     help="cartella di destinazione in cui scrivere domain.rddl e "
                          "instance.rddl (creata se non esiste). Default: ./src/output")
    ...
    ap.add_argument('--initial-col', default='initial',
                     help="nome della colonna booleana nel CSV degli stati che flagga gli "
                          "stati candidati come iniziali (default: 'initial'). Se presente e "
                          "--init-state non e' passato, lo script chiede interattivamente "
                          "all'utente quale stato usare (se i candidati sono piu' di uno).")
    ...
    args = ap.parse_args()
    # se --states/--transitions sono gia' percorsi assoluti (o comunque
    # contengono una directory), os.path.join li rispetta cosi' come sono
    # solo se args.states e' assoluto; per percorsi relativi con sottocartelle
    # (es. 'data/states.csv') vengono comunque risolti rispetto a --indir.
    states_path = os.path.join(args.indir, args.states)
    transitions_path = os.path.join(args.indir, args.transitions)
    ...
    # la colonna 'initial' (o quella indicata da --initial-col) e' usata solo
    # per la selezione dello stato iniziale, non deve diventare un attributo
    skip_cols = {args.last_action_col, args.initial_col}
    ...
    # stato iniziale: priorita' a --init-state, poi selezione (interattiva se
    # necessario) via colonna 'initial', poi fallback storico su
    # last_action_col == 'start', infine stato 0
    init_idx = choose_init_state(states_df, args.id_col, args.initial_col, args.init_state)
    if init_idx is None:
